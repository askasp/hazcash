defmodule HazCash.Accounts do
  ...
  alias HazCash.Accounts.Membership
  alias HazCash.Accounts.Member
  alias HazCash.Accounts.Invitation

  def paginate_accounts(params \\ %{}) do
    HazCash.Pagination.paginate_records(Account, params)
  end
  ...
  alias HazCash.Mailer
  alias HazCashWeb.Router.Helpers, as: Routes
  alias HazCash.Accounts.InvitationNotifier
  ...
  def invite_member({:ok, %{email: email} = _invitation} = result) do
    url =  Routes.user_session_url(HazCashWeb.Endpoint, :new)
