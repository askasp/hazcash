defmodule HazCashWeb.Admin.ResetPasswordController do
  use HazCashWeb, :controller

  alias HazCash.{Admins, Admins.AdminNotifier, Mailer, Admins.Guardian}

  plug HazCashWeb.Plugs.RedirectAdmin
  ...
  def create(conn, %{"admin" => %{"email" => email}}) do
    if admin = Admins.get_admin_by_email(email) do
      token = Phoenix.Token.sign(HazCashWeb.Endpoint, "admin_auth", admin.id)
  ...
  def show(conn, %{"token" => token}) do
    case Phoenix.Token.verify(HazCashWeb.Endpoint, "admin_auth", token, max_age: 600) do
      {:ok, id} ->
