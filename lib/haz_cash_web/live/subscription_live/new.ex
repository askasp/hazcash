defmodule HazCashWeb.SubscriptionLive.New do
  @moduledoc """
  ...
  import HazCash.Billing.Helpers, only: [encode_token: 1]
  alias HazCash.Billing

  on_mount {HazCashWeb.Live.SubscriptionAssigns, :billing_customer}
  ...
  def handle_event("checkout", _, socket) do
    ...
    return_url = Routes.stripe_return_url(socket, :new)
    ...
    {:ok, %{id: stripe_session_id, payment_intent: _stripe_id}} =
      Stripe.Session.create(%{
        ...
        mode: "subscription",
        success_url: "#{return_url}?t=success&bp=#{encoded_token}",
        cancel_url: "#{return_url}?t=cancel&bp=#{encoded_token}"
      })
