defmodule HazCashWeb.TransferLive.FormComponent do
  ...
  def update(%{transfer: transfer} = assigns, socket) do
    changeset = Core.change_transfer(transfer)
  ...
  def handle_event("validate", %{"transfer" => transfer_params}, socket) do
    changeset =
      ...
      |> Core.change_transfer(transfer_params)
  ...
  defp save_transfer(socket, :edit, transfer_params) do
    case Core.update_transfer(socket.assigns.transfer, transfer_params) do
      {:ok, _transfer} ->
        {:noreply,
         socket
  ...
  defp save_transfer(socket, :new, transfer_params) do
    case Core.create_transfer(transfer_params) do
      {:ok, _transfer} ->
        {:noreply,
         socket
